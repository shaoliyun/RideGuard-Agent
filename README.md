...
本项目是在原有 `taxi-agent` 项目基础上的持续改造版本。原项目已经具备多 Agent 路由、订单处理、工具调用、分层记忆和 RAG 等核心业务能力；本阶段重点不是重新实现这些功能，而是围绕安全、可靠性、可观测性和可扩展性补齐工程化能力。
...
- **配置安全**：增加无秘密的环境变量配置模板，避免将有效凭据提交到代码仓库。
- **容器化运行**：增加 `Dockerfile` 和 `Compose` 基础设施配置，为 MySQL、Redis、MongoDB 和 Elasticsearch 提供统一的本地启动方式。
- **订单一致性**：引入显式订单状态、白名单状态迁移、Redis 原子迁移和重复创建防护，降低并发确认和重复下单风险。
- **基础可观测性**：接入 Spring Boot Actuator 和 Prometheus，提供健康检查与指标暴露基础。
- **Capability 抽象**：统一描述能力名称、风险级别、权限、确认要求、输入输出类型和执行上下文，并通过 Registry 校验能力名称唯一性。
- **外部依赖韧性**：引入 Resilience4j 配置基础，区分只读调用与写操作的重试策略，为超时、熔断、限流和降级提供统一落点。
- **自动化验证**：增加订单状态机、Capability Registry 和首个知识库 Capability 的确定性测试。

### 演进目标

本项目希望从“能够完成网约车业务流程的 Agent 应用”继续演进为“面向复杂出行任务的可恢复、可审计 Agent 平台”，重点目标包括：

1. 让开发者可以通过示例配置和 Compose 快速复现完整运行环境。
2. 让订单确认和创建具备明确状态、原子迁移和幂等保护。
3. 让关键请求、工具调用和业务状态迁移具备统一的上下文与指标。
4. 让地图、天气、车型和通知等能力可以通过 Capability 和 Provider 接口扩展。
5. 为轮椅、孕妇、老人、携童、宠物和大件行李等特殊出行需求保留风险、权限和人工确认能力。

当前仍处于渐进改造阶段，存量工具尚未全部迁移为 Capability，Provider 端口和 `AgentEvent` 协议版本化也将在后续迭代中完成。
...
- **生产化基础**：环境变量配置、Docker Compose、订单状态机、Actuator/Prometheus 和外部依赖韧性配置
- **可扩展能力模型**：Capability Registry 支持风险、权限、确认和执行上下文元数据
